...
#' We can group_by island and species now, with the same summaries. 
penguins %>%  group_by(island, species) %>% drop_na() %>% 
  summarize(mean_bill_length_mm = mean(bill_length_mm), 
            max_bill_length_mm = max(bill_length_mm))

#' We can filter results using the filter() 
#' Let's say only want data on the daily penguins. 
penguins %>% filter(species == "Adelie") %>% group_by(island, species) %>% drop_na() %>% 
  summarize(mean_bill_length_mm = mean(bill_length_mm), 
            max_bill_length_mm = max(bill_length_mm))

#' This just to keep as reference but the message output >
#'summarise() has grouped output by 'island'. You can override using 
#'the `.groups argument.
#'this just means:
#'After summarizing, species was dropped as a grouping variable since 
#'it's the same in all rows.
#'If you want to drop all grouping, you can add .groups = "drop":
#'Search R GPT for utilization efficiency. 